...
app.post('/signup', async (req, res) => {
    try {
        ...
        if (existingMail) { return res.status(409).json({ error: 'Email already exists' }); }
...
app.post('/getTask', authenticateToken, async (req, res) => {
    try {
        const currentUser = await User.findOne({ _id: req.user.userId }, { name: 1, email: 1, Todo: 1, Ongoing: 1, Done: 1, Expired: 1 });
        if (!currentUser) {
...
app.post('/editTask', authenticateToken, async (req, res) => {
    try {
        ...
        const { type, cardId, title, priority, description, deadline = null } = req.body;
...
app.post('/moveTask', authenticateToken, async (req, res) => {
    try {
        ...
        const userDetails = await User.findOne(
            { _id: currentUser._id, [currentType]: { $elemMatch: { cardId: cardId } } },
